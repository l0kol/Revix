...
const DashboardPage: React.FC = () => {
  ...
  const { ipRegistrations, isLoadingRegistration, registerIp } = useStoryProtocol();
  ...
  if (!isWalletConnected) { 
    return (
      <div className="text-center py-10">
        <p className="text-xl text-creator-text-secondary">Please connect your wallet to view your videos and manage IP.</p>
      </div>
    );
  }
  ...
  if (videosError) {
    return <p className="text-red-500 text-center py-10">Error fetching videos: {videosError}</p>;
  }

  return (
    <div className="container mx-auto"> 
      <h2 className="text-3xl font-bold text-creator-text-primary mb-6">My YouTube Videos</h2>
      ...
      {processedVideos.length === 0 && !isLoadingVideos && (
        <p className="text-creator-text-secondary text-center py-10">
          {searchTerm ? 'No videos match your search.' : 'No videos found. Ensure your YouTube channel is connected and has videos.'}
        </p>
      )}
      ...
      {selectedVideo && ipRegistrations[selectedVideo.id]?.status === IPRegistrationStatus.REGISTERED && (
         <Modal
